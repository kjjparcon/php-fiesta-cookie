<?php
{
	echo '<b>Pokemon Defense:</b>';
}

function pokemon_defense($headache, $limit_number){					#if statement within a for loop to check each row's species
		for($i = 1;$i <= 10;$i++){									#on whether or not their defense is >= to the limit_number	
			if($headache[$i]['defense'] >= $limit_number){			
				echo '<br>'.$headache[$i]['species']."'s defense is greater than or equal to"." ".$limit_number;
			}
		}
}
		#multi-dimensional array and coder's given limit_number inside pokemon_defense
